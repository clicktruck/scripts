  ...
  if [ -z "$4" ]; then
    ...
    if [ -z "$3" ]; then
      ...
      if [ -d "${GITOPS_DIR}/.init" ] && [ -d "${GITOPS_DIR}/.install" ]; then
        ...
        echo "Expected to find both .init and .install sub-directories underneath $GITOPS_DIR"
        exit 1
      fi
        ...
        if [ $files -eq 1 ]; then
          ...
          if [ "$kind" == "App" ]; then
            ...
            if [ $i -gt 0 ] && [ $i -eq $ytt_path_count ]; then
              ...
              # Allow for additional phase of post-install
              # Execute script if it exists; then re-deploy
              kicks=$(find ${GITOPS_DIR}/.post-install -type f -name "kick.sh" | wc -l)
              if [ $kicks -eq 1 ]; then
                cd ${GITOPS_DIR}/.post-install && ./kick.sh
              fi
            fi
          fi
        fi
